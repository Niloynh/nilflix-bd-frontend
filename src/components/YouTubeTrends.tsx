...
export default function YouTubeTrends() {
    ...
    const fetchVideos = async () => {
      ...
      try {
        const res = await fetch(
          `${apiUrl}/api/videos?source=YouTube&isShort=false&limit=5`
        );
  ...
  if (loading)
    return <p className="text-gray-400 my-8">Loading YouTube Trends...</p>;
  ...
  return (
    <section className="my-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/youtube" className="text-sm text-red-500">
          See All
        </Link>
      </div>
      ...
      <div className="space-y-6">
        {videos.map((video: any) => {
          ...
          return (
            <div key={video._id} className="group block">
              ...
              <div className="flex items-start gap-3 mt-3">
                <div
                  className={`flex-shrink-0 w-9 h-9 ${avatarColor} rounded-full flex items-center justify-center font-bold text-white`}
                >
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-xs mt-1">
                    {video.channelTitle} •{" "}
                    {formatViews(video.sourceStats?.views)} •{" "}
